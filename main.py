...
tile_size = (width/grid[0] - width/(grid[0]+(grid[0]*4)), height/grid[1] - height/(grid[1]+(grid[1]*2.6)) )
...
def choose_target(seed):
    ...
    for i in range(len(seed)):
        print("############", seed[i])
    return pygame.transform.scale( pygame.image.load (io.BytesIO (urlopen(images[int(seed[random.randint(0,len(seed)-1)])][1].replace(' ,', '')).read())) ,tile_size)
...
show_target = pygame.transform.scale(pygame.image.load('show_button.png'),(100,70))
show_target_rect = pygame.Rect((0,screen_size[1]-500),(100,70))

image_blocker = pygame.transform.scale(pygame.image.load('Anchoine.jpg'),tile_size)
#########################################################################   GAME LOOP   #########################################################################
...
while running:
    ...
    display(show_target,(0,screen_size[1]-500))
    ...
    if event.type == pygame.MOUSEBUTTONDOWN:
        if event.button == 1:
            ...
            if seed_button.collidepoint(pos):
                ...
                seed_choice = choose_target( pyperclip.paste().replace('[','').replace(']','').split(', ') )
                display(seed_choice, (0,screen_size[1]-400))
                print(pyperclip.paste())
            elif show_target_rect.collidepoint(pos):
                display(image_blocker, (0,screen_size[1]-400))
                print("Target Displayed")
        elif event.button == 3:
            pos = pygame.mouse.get_pos()  # Obtient la position de la souris
            for rect in tiles:
                if rect[0].collidepoint(pos):  # Vérifie si le rectangle est cliqué
                    print("Blocked")
                    display(rect[1], rect[0].topleft)
            if show_target_rect.collidepoint(pos):
                display( seed_choice, (0,screen_size[1]-400))
                print("Target Shown")
        elif event.button == 2:
